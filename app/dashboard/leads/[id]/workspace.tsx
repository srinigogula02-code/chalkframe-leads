...
import { AlertTriangle, ArrowLeft, ArrowRight, ArrowUpRight, Bot, Check, CheckCircle2, Clock3, Copy, Image as ImageIcon, Images, Mail, Phone, Plus, RefreshCw, Save, Sparkles, StickyNote, Trash2, X } from "lucide-react";
...
export default function BusinessWorkspace({ user, lead, previousId, nextId, statusFilter }: { user: SessionUser | null; lead:BusinessLead; previousId:string|null; nextId:string|null; statusFilter:WorkflowStatus|"all" }) {
  ...
  return <main className="business-page">
    ...
    <div className="business-layout"><div className="business-canvas">
      ...
      {redesigns.some(image=>image.url)&&<section className="business-section comparison-section"><div className="section-heading"><div><span className="technical">Automatic output</span><h2>Collages + email drafts</h2></div><span>16:9 · no cropping</span></div><div className="comparison-grid">{redesigns.filter(image=>image.url).map((image,index)=><CollageCard key={image.id||index} image={image} index={index} hasOriginal={originals.length>0} selectedOriginal={Boolean(selectedOriginal)} onOpen={showImage} onRetry={()=>void queueCollages(selectedOriginal,true)} onRegenerate={()=>void regenerateEmail(image)}/>)}</div></section>}
