...
export default async function DashboardPage({ searchParams }: { searchParams: Promise<{ status?: string }> }) {
  ...
  const initialTab = user.role === "admin" && isWorkflowStatus(requestedStatus) ? requestedStatus : "all";
  ...
  return <Dashboard initialLeads={leads as unknown as Lead[]} initialUsers={users as unknown as TeamUser[]} user={user} initialTab={initialTab} />;
}
