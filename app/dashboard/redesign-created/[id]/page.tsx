...
export type RedesignLead = { id: string; title: string | null; ad_url: string; email: string | null; workflow_status: string; created_at:string; images: ReviewImage[]; redesign_images: ReviewImage[] };
...
export default async function RedesignCreatedBusinessPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const rows = await sql`SELECT l.id, l.title, l.ad_url, l.email, l.workflow_status, l.created_at,
