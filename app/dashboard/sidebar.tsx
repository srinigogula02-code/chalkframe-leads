...
export default function DashboardSidebar({ user, active }: { user: SessionUser; active: Section }) {
  const navigation = (
    <>
      <Link prefetch={false} className={`nav-item ${active === "leads" ? "active" : ""}`} href="/dashboard">
        <LayoutGrid size={18} />Leads
      </Link>
      {user.role === "admin" && (
        <>
          <Link prefetch={false} className={`nav-item ${active === "research" ? "active" : ""}`} href="/dashboard/research-completed">
            <CheckCircle2 size={18} />Research completed
          </Link>
          <Link prefetch={false} className={`nav-item ${active === "redesign" ? "active" : ""}`} href="/dashboard/redesign-created">
            <Palette size={18} />Redesign created
          </Link>
          <Link prefetch={false} className={`nav-item ${active === "ai" ? "active" : ""}`} href="/dashboard/ai">
            <Bot size={18} />AI email drafts
          </Link>
          ...
          <Link prefetch={false} className="nav-item" href="/dashboard#team">
            <Users size={18} />Team earnings
          </Link>
        </>
      )}
    </>
  ...
  return (
    <aside className="sidebar">
      <img src="/brand/chalkframe-logo-dark.svg" alt="Chalkframe" className="side-logo" />
      <nav className="desktop-nav">{navigation}</nav>
      <details className="mobile-nav">
        <summary aria-label="Open sidebar navigation">
